...
def predict():
    ...
    try:
        ...
        if len(response["detected_conditions"]) > 0:
            response["primary"] = "Abnormal - Conditions Detected"
            response["description"] = f"The analysis detected: {', '.join(response['detected_conditions'])}"
        else:
            response["primary"] = "Normal - No Conditions Detected"
            response["description"] = "No abnormal conditions were detected in this X-ray image."
