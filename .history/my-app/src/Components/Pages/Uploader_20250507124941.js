    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            const formattedResults = {
                ...
                probabilities: [],
                detectedConditions: [] // Will store detected conditions
            };

            // Get detected conditions directly from backend response
            if (results.detected_conditions && Array.isArray(results.detected_conditions)) {
                formattedResults.detectedConditions = results.detected_conditions;
            }
            ...
            if (results.predictions) {
                ...
                formattedResults.probabilities = Object.entries(results.predictions)
                    .map(([label, probability]) => {
                        // Check if this condition is in the detected_conditions array
                        const isDetected = results.detected_conditions && 
                                          results.detected_conditions.includes(label);
                        
                        return {
                            label: label.charAt(0).toUpperCase() + label.slice(1), // Capitalize label
                            percentage: Math.round(probability * 100),
                            detected: isDetected // Mark if detected based on backend thresholds
                        };
                    })
            ...
            {analysisResults && (
                <div className="analyzer-right-panel">
                    <div className="results-card">
                        ...
                        {/* Replace the high confidence findings section with: */}
                        <div className="all-probabilities">
                            <h3>Detected Conditions (Using Optimal Thresholds):</h3>
                            <div className="probability-list">
                                {analysisResults.detectedConditions && 
                                 analysisResults.detectedConditions.length > 0 ? (
                                    analysisResults.probabilities
                                        .filter(item => item.detected) // Only show items detected by the backend
                                        .map((item, index) => (
