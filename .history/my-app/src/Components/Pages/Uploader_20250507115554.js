...
const Uploader = () => {
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            // Process predictions if available - only include conditions with non-zero probabilities
            if (results.predictions) {
                formattedResults.probabilities = Object.entries(results.predictions)
                    .filter(([label, probability]) => probability > 0.05) // Only include if probability > 5%
                    .sort((a, b) => b[1] - a[1])
